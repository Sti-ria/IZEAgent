...
def solve(context: BreakContext) -> BreakPlan:
    ...
    for row in range(5):
        ...
        for act in base_actions:
            # 核心逻辑：如果推荐的是铁桶(bucket)，因为血量悬殊，降级为【两个路障】打消耗
    ...
    # -------------------------------------------------------------
    # 第二步：舞王全局扫描（修改：存在连续三路花费之和 > 350 时，只接受在中间路下舞王）
    # -------------------------------------------------------------
    ...
    for start_row in range(3):
        ...
        if total_three_lane_cost > DANCER_COST_THRESHOLD:
            ...
            # 只有当中间行本身也完美符合放舞王的环境时，才执行舞王策略
            if can_dance:
                display_row = target_row + 1
                dance_actions = []
                
                if dance_type == "single":
                    dance_actions.append(BreakAction(zombie="dancer", row=target_row, count=1, note=f"R{display_row}: 连续3路总开销{total_three_lane_cost}>350，在中间路单放舞王"))
                    reasons[target_row] = f"R{display_row}: 连续高总价中间路单下舞王"
                elif dance_type == "cone":
                    dance_actions.append(BreakAction(zombie="cone", row=target_row, count=1, note=f"R{display_row}: 连续高总价且3输出，中间路先下【路障】垫刀"))
                    dance_actions.append(BreakAction(zombie="dancer", row=target_row, count=1, note=f"R{display_row}: 确认路障死亡，补放舞王"))
                    reasons[target_row] = f"R{display_row}: 连续高总价中间路路障+舞王"
    ...
    return BreakPlan(
        ...
        confidence=0.96,
        reason="穿刺阵最新策略: " + " | ".join(filter(None, reasons))
    )